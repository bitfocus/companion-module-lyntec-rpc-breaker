...
module.exports = {
	...
	processBytes: function(bytes){
		...
		if(bytes[0] == 0xC8){ //breaker Status
			//B0 C8 1F 02 F0
		...
		if(bytes[0] == 0xB6){ //All Breaker Status
		...
		if(bytes[0] == 0xB9){ //All Zone Status	
	...
	requestZones: function() {//request zone status
		let self = this;
		self.commands.push("B0B9F0");
	},
	
	requestBreakers: function() {//Request breaker status
		let self = this;
